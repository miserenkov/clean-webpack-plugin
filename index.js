...
var clean = function() {
    ...
    _this.paths.forEach(function(rimrafPath) {
        ...
        if (_this.options.exclude && _this.options.exclude.length) {
            _this.options.exclude = _this.options.exclude.map(function (item) {
                return path.resolve(rimrafPath, item);
            });

            try {
                var pathStat = fs.statSync(rimrafPath);
                if (pathStat.isDirectory()) {
                    childrenAfterExcluding = glob.sync(rimrafPath + '/**/*')
                        .filter(function(childFile) {
                            childFile = path.resolve(rimrafPath, childFile);
                            var include = _this.options.exclude.indexOf(childFile) < 0;
                            if (!include) {
                                excludedChildren.push(childFile);
                            }
                            return include && !fs.lstatSync(childFile).isDirectory();
                        })
                        .map(function(file) {
                            var fullPath = path.resolve(rimrafPath, file);
                            if (os.platform() === 'win32') {
        ...
        console.warn('clean-webpack-plugin: ' + excludedChildren.length + " file(s) excluded - \n\t" + excludedChildren.join(",\n\t"));
